...
if simParameters.PDSCH.NLayers > 4              % Multicodeword transmission
    ...
    simParameters.PDSCH.TargetCodeRate = [500 500]./1024; % Code rate used to calculate transport block sizes
    simParameters.PDSCH.Modulation = {'64QAM','64QAM'};   % 'QPSK', '16QAM', '64QAM', '256QAM'
    simParameters.NRxAnts = 8;                            % Number of UE receive antennas
else
    simParameters.NumCW = 1;                        % Number of codewords
    simParameters.PDSCH.TargetCodeRate = 500/1024;  % Code rate used to calculate transport block sizes
...
% Toolbox is not installed, 'parfor' defaults to normal 'for' statement
    ...
    while gnb.NSymbol < NSymbols  % Move to next slot, gnb.NSymbol increased in steps of one slot               
        ...
        for cwIdx = 1:gnb.NumCW
            ...
            if NDI 
                trBlk = randi([0 1],floor(trBlkSizes(cwIdx)/3),1);
                %LDPC Encoding
                %cword = nrldpc_encode(H_256,256,codedTrBlock);
                filler = -1*ones(384*22-mod(length(trBlk),384*22),1);
                %trBlk = [trBlk;filler];
                code = [];
                n_trBlk = [];
                last = 0;
                out_r = 384*22*2;
                rv2 = 0;%harqProcesses(harqProcIdx).RV;
                nLayers_rm = 1;
                for i = 1:floor(length(trBlk)/(22*384))
                    cword = trBlk(((i-1)*384*22+1):((i)*384*22));
                    n_trBlk = [n_trBlk;cword];
                    ld = [nrLDPCEncode(cword,1)];
                    ld_rm = nrRateMatchLDPC(ld,out_r,rv2,'QPSK',nLayers_rm);
                    code = [code;ld_rm];
                    last = i;
                end    
                
                code = [code;trBlk(last*22*384+1:end)];
                %cword = [cword];
                %{
                n_trBlk = [n_trBlk;cword];
                ld = [nrLDPCEncode(cword,1)];
                code = [code;ld];
                %} 
                setTransportBlock(encodeDLSCH,code,cwIdx-1,harqProcIdx-1);
            end
        end
        ...
        nlayers_rm = 1;            
        numCB_rm = 1;
        decodeDLSCH.TransportBlockLength = length(code);%trBlkSizes;
        [out_r,harqProcesses(harqProcIdx).blkerr] = decodeDLSCH(dlschLLRs,pdsch.Modulation,pdsch.NLayers,harqProcesses(harqProcIdx).RV,harqProcIdx-1);
        last = 0;
        rv2 = 0;%harqProcesses(harqProcIdx).RV;
        for i = 1:floor(length(out_r)/(22*384*R_in))
            out = nrRateRecoverLDPC(single(out_r(((i-1)*(384*22*R_in)+1):((i)*(384*22*R_in)))),384*66*4,R_rm,rv2,mod_rm,nlayers_rm,numCB_rm);
            decbits = [decbits;nrLDPCDecode(single(out),1,5)];
            last = i;
            %disp(size(decbits));
        end
        %disp(size(out_r((last*22*384*R_in+1):end)));
        decbits = [decbits;out_r((last*22*384*R_in+1):end)];

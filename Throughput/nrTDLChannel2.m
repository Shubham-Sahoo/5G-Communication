classdef (StrictDefaults)nrTDLChannel2 < matlab.System 
%nrTDLChannel TR 38.901 Tapped Delay Line (TDL) channel
...
%   single precision data type scalar, vector, or 2D matrix. X is of size
...
%   Ns-by-Np-by-Nt-by-Nr, where Np is the number of paths, i.e., the length
%   of the PathDelays property value of CHAN. PATHGAINS is of the same
%   precision as input X.
%
...
%   FadingDistribution         - Rayleigh or Rician fading or nakagami
...
%   % waveform spectrum. Use TDL-C delay profile, 300ns delay spread and UT
%   % velocity 30km/h.
%   
...
%   % signal. Use TDL-D delay profile, 10ns delay spread and a desired
%   % overall K-factor of 7.0dB. Configure cross-polar antennas according
...
%   rxWaveform = tdl(txWaveform);
%   
%   See also nrCDLChannel, comm.MIMOChannel2, nrPerfectTimingEstimate,
...
%   public interface
    ...
    methods (Access = public)
        ...
        function obj = nrTDLChannel2(varargin)
        ...
        %   called once.
            ...
            h = nrTDLChannel2.makePathFilters(obj);            
    ...
    properties(Hidden,Transient)
        ...
        FadingDistributionSet = matlab.system.StringSet({'Rayleigh','nakagami'});
...
%   protected interface
    ...
    methods (Access = protected)
        ...
        function setupImpl(obj)
            ...
            % Construct the MIMOChannel2
            obj.theChannel = nrTDLChannel2.makeMIMOChannel2(obj);
        ...
        function varargout = stepImpl(obj,in)
            ...
            % execute the MIMOChannel2
            [varargout{1:min(nargout,2)}] = obj.theChannel(in);
            
            % create sample times output if required
            if (nargout==3)
                theInfo = info(obj.theChannel);
                Nend = theInfo.NumSamplesProcessed;
                Ns = size(varargout{2},1);
                varargout{3} = (Nend-Ns:Nend-1).'/obj.SampleRate;
            end
        ...
        %   SpatialCorrelationMatrix - Combined correlation matrix (or 3-D array).
            ...
            if (~isempty(coder.target) || ~isLocked(obj))
                nrTDLChannel2.validation(obj);
                channel = nrTDLChannel2.makeMIMOChannel2(obj);
            else
        ...
        function flag = isInactivePropertyImpl(obj,prop)
            ...
            elseif (strcmp(prop,'KFactorFirstTap'))
                flag = ~strcmp(obj.DelayProfile,'Custom') || ~strcmp(obj.FadingDistribution,'nakagami'); 
...
%   private
    ...
    methods (Static, Access = private)
        ...
        function c = makeMIMOChannel2(obj)
            
            c = MIMOChannel2;
            c.SampleRate = obj.SampleRate;
            [pathDelays,averagePathGains,K_1dB] = nrTDLChannel2.getDelayProfile(obj);
            ...
            %{ 
                else if (nrTDLChannel.hasLOSPath(obj))
                c.FadingDistribution = 'nakagami';
                c.KFactor = 10^(K_1dB/10); % convert dB to linear
                c.DirectPathDopplerShift = 0.7*obj.MaximumDopplerShift;
                c.DirectPathInitialPhase = 0.0;    
            ...
            c.NumTransmitAntennas = nrTDLChannel2.getAntennaCount(obj);
            ...
            c.SpatialCorrelationMatrix = nrTDLChannel2.getSpatialCorrelationMatrix(obj);
        ...
        function Rspat = getSpatialCorrelationMatrix(obj)
            ...
            if (strcmp(obj.MIMOCorrelation,'Custom') && strcmp(obj.Polarization,'Custom'))
                ...
                P = nrTDLChannel2.getPermutationMatrix(obj);
                ...
                % 1-by-1(-by-Np)
                Rt = nrTDLChannel2.getTransmitCorrelationMatrix(obj);
                Rr = nrTDLChannel2.getReceiveCorrelationMatrix(obj);
                ...
                Gamma = nrTDLChannel2.getPolarizationCorrelationMatrix(obj);
                
                % Compute overall spatial correlation matrix. Rspat is of
                % size (Nt*Nr)-by-(Nt*Nr)(-by-Np)
                a = nrTDLChannel2.getRoundoffScalingFactor(obj);
        ...
        function P = getPermutationMatrix(obj)
            ...
            [Nt,Nr] = nrTDLChannel2.getAntennaCount(obj);
            ...
            if (strcmp(obj.MIMOCorrelation,'Custom'))
                if (strcmp(obj.Polarization,'Custom'))
                    ...
                    if (strcmp(obj.Polarization,'Cross-Polar'))
                        [txPolAngles,rxPolAngles] = nrTDLChannel2.getPolarizationAngles(obj);
        ...
        function Rt = getTransmitCorrelationMatrix(obj)
            ...
            if (strcmpi(obj.MIMOCorrelation,'Custom'))
                ...
                switch (nrTDLChannel2.getMIMOCorrelationVersusLink(obj))
                    case 'Low'
                        r = 0;
                ...
                Rt = nrTDLChannel2.getCorrelationMatrix(obj,obj.NumTransmitAntennas,r);
            end
        ...
        function Rr = getReceiveCorrelationMatrix(obj)
            ...
            if (strcmpi(obj.MIMOCorrelation,'Custom'))
                ...
                Rr = nrTDLChannel2.getCorrelationMatrix(obj,obj.NumReceiveAntennas,r);
            end
            ...
            switch (obj.Polarization)
                ...
                case 'Cross-Polar'
                    ...
                    if (strcmp(obj.MIMOCorrelation,'Custom'))
                        ...
                        gamma = nrTDLChannel2.getPolarizationCorrelation(obj);
        ...
        function has = hasLOSPath(obj)
            ...
            if strcmpi(obj.DelayProfile,'Custom')
                has = strcmpi(obj.FadingDistribution,'nakagami');
            %{
            else if strcmpi(obj.DelayProfile,'Custom')
                has = strcmpi(obj.FadingDistribution,'nakagami');
            %}
            else
        ...
        function validation(obj)
            ...
            Np = size(nrTDLChannel2.getDelayProfile(obj),2);
            nrTDLChannel2.validateCorrelationMatrixSize(nrTDLChannel2.getTransmitCorrelationMatrix(obj),Np,'TransmitCorrelationMatrix');
            nrTDLChannel2.validateCorrelationMatrixSize(nrTDLChannel2.getReceiveCorrelationMatrix(obj),Np,'ReceiveCorrelationMatrix');

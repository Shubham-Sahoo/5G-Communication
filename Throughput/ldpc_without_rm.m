...
% Toolbox is not installed, 'parfor' defaults to normal 'for' statement
    ...
    while gnb.NSymbol < NSymbols  % Move to next slot, gnb.NSymbol increased in steps of one slot               
        ...
        [out ,harqProcesses(harqProcIdx).blkerr] = decodeDLSCH(dlschLLRs,pdsch.Modulation,pdsch.NLayers,harqProcesses(harqProcIdx).RV,harqProcIdx-1);
        last = 0;
        decbits = [];
        for j = 1:floor(length(out)/(66*384))
            decbits = [decbits;nrLDPCDecode(double(out((((j-1)*384*66)+1):((j)*384*66))),1,5)];
            last = j;
            %disp(size(decbits));
        end
        decbits = [decbits;out(((last*66*384)+1):end)];

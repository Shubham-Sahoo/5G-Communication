...
simParameters.SNRIn = [5,10,20,40,60,80,100,120,130,170,200];%(-20:30);%[-20 -15 -13 -10 -7 -3 0 3 6 10 15]; % SNR range (dB)
...
code_rates = [120,240,300,400,500,600,700,800,900]./1024;
...
for modIdx = 1:numel(moduls)
    for codIdx = 1:numel(code_rates)
            ...
            simParameters.PDSCH.PRBSet = 0:simParameters.NRB-1;  %0:simParameters.NRB-1; % PDSCH PRB allocation
            ...
            if simParameters.PDSCH.NLayers > 4              % Multicodeword transmission
                ...
                simParameters.PDSCH.TargetCodeRate = [code_rates(codIdx) code_rates(codIdx)]; % Code rate used to calculate transport block sizes
                simParameters.PDSCH.Modulation = {moduls(modIdx),moduls(modIdx)};   % 'QPSK', '16QAM', '64QAM', '256QAM'
                simParameters.NRxAnts = 8;                            % Number of UE receive antennas
            else
                simParameters.NumCW = 1;                        % Number of codewords
                simParameters.PDSCH.TargetCodeRate = code_rates(codIdx);  % Code rate used to calculate transport block sizes
                simParameters.PDSCH.Modulation = moduls(modIdx);       % 'QPSK', '16QAM', '64QAM', '256QAM'
            ...
            simParameters.SSBurst.SSBPeriodicity = 30;        % SS burst set periodicity in ms (5, 10, 20, 40, 80, 160)
            ...
            elseif strcmpi(channelType,'TDL')
                channel = nrTDLChannel2; % TDL channel object
                % Set the channel geometry
                channel.DelayProfile = 'Custom';        %Custom
                channel.DelaySpread = 300e-9;
        ...
        for snrIdx = 1:numel(snrIn) 
            ...
            if(dist_2d<=18)
                ...
                prob_los = (18/dist_2d)+exp(-dist_2d/36)*(1-18/dist_2d);
            ...
            for loop = 1:loop_time    
                % comment out for parallel computing
            ...
            % Toolbox is not installed, 'parfor' defaults to normal 'for' statement
                ...
                while gnb.NSymbol < NSymbols  % Move to next slot, gnb.NSymbol increased in steps of one slot               
                    ...
                    % is defined per RE for each receive antenna (TS
                    % 38.101-4).4
                    ...
                    p_unlos = 22.4+35.3*log10(dist_3d)+21.3*log10(frequency)-0.3*(h_ut-1.5);
                    if(dist_2d>=10 && dist_2d<d_bp)
                        p_los = 32.4+21*log10(dist_3d)+20*log10(frequency);
                        p_nlos = max(p_los,p_unlos);
                    else
                        p_los = 32.4+40*log10(dist_3d)+20*log10(frequency)-9.5*log10(power(d_bp,2)+power(h_bs-h_ut,2));
                        p_nlos = max(p_los,p_unlos);
                    end
                    ...
                    SINR_NLOS =((10^(16/10)/51)*10^(Tot_gain_NLOS/10)*G_T*G_R)/(10^-(14.84));      %Considering only the noise PSD -174dBm/Hz, noise power per RB=-148.365dB
                ...
                if(loop==1)
                    ...
                    bit_error(snrIdx) = mean(bit_err)/loop_time;
                else
                    ...
                    bit_error(snrIdx) = bit_error(snrIdx)+mean(bit_err)/loop_time;
                end
                ...
                %disp(size(trBlk));
                %disp(maxThroughput);
                thr = simThroughput(snrIdx)/loop_time;
                %disp(thr);
                %disp(thr(1));
            ...
            if(simParameters.PDSCH.Modulation=="QPSK")
                modl = 0;
                modll = cat(1,modl);
                trcc = cat(1,trc);
                snrr = cat(1,SINRdB_TOT);
                thrr = cat(1,thr(1));
                berr = cat(1,bit_error(snrIdx));
                dis = cat(1,dist_2d);
                csi_ber = cat(2,berr);   %cat(2,csi{1,1},berr);
                mat_res = cat(2,csi_ber,thrr,snrr,trcc,modll,dis);
                mat_res = transpose(mat_res);
                dlmwrite('csi_dataQPSK.csv',mat_res,'-append');
            elseif(simParameters.PDSCH.Modulation=="16QAM")
                modl = 1;
                modll = cat(1,modl);
                trcc = cat(1,trc);
                snrr = cat(1,SINRdB_TOT);
            ...
            elseif(simParameters.PDSCH.Modulation=="64QAM")
                modl = 2;
                modll = cat(1,modl);
                trcc = cat(1,trc);
                snrr = cat(1,SINRdB_TOT);
            ...
            %writematrix(mat_res,'csi_data.csv');
        end

    ...
    while gnb.NSymbol < NSymbols  % Move to next slot, gnb.NSymbol increased in steps of one slot               
        ...
        for cwIdx = 1:gnb.NumCW
            ...
            if NDI 
                ...
                n_trBlk = [];
                last = 0;
                for i = 1:floor(length(trBlk)/(6144))

                    cword = trBlk(((i-1)*6144+1):((i)*6144));
                    n_trBlk = [n_trBlk;cword];
                    ld = lteTurboEncode(cword);
                    %ld = [nrLDPCEncode(cword,1)];
                    code = [code;ld];
                    last = i;
                end    
                
                code = [code;trBlk(last*6144+1:end)];
                %cword = [cword];
                %{
                n_trBlk = [n_trBlk;cword];
                ld = [nrLDPCEncode(cword,1)];
        ...
        last = 0;
        for i = 1:floor(length(out)/(18444))
            decbits = [decbits;lteTurboDecode(double(out(((i-1)*18444+1):((i)*18444))),5)];
            last = i;
        end
        decbits = [decbits;out(last*18444+1:end)];
